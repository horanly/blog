...
<<< @/tpl/cross-domain/jsonp原生js实现跨域.js
...
~~~ js
this.$http = axios;
this.$http('http://www.domain2.com:8080/login', {
    param:{},
...
<<< @/tpl/cross-domain/jsonp后端node跨域.js

> jsonp的缺点： 只能发送get一种请求

### 2、跨域资源共享（CORS）

CORS 是一个W3C标准，全称‘跨域资源共享’（Cross-origin resource sharing）。它允许浏览器向跨域服务器发送 XMLHttpRequest请求，从而克服了ajax只能同源使用的限制。
...
<<< @/tpl/cross-domain/cors原生js实现跨域.js
...
<<< @/tpl/cross-domain/cors后端node实现跨域.js
...
<<< @/tpl/cross-domain/nginx配置实现跨域.js
...
<<< @/tpl/cross-domain/node中间件实现跨域.js
...
<<< @/tpl/cross-domain/WebSocket后端node实现跨域.js
